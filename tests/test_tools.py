...
def test_fs_write_creates_backup(tmp_path):
    ...
    out = agent_tools.fs_write(
        {"command": "str_replace", "path": str(f),
         "old_str": "hello", "new_str": "world"}, cwd=str(tmp_path))
...
def test_fs_write_create_no_backup_for_new(tmp_path):
    f = tmp_path / "new.txt"
    out = agent_tools.fs_write(
        {"command": "create", "path": str(f), "file_text": "x"},
        cwd=str(tmp_path))
...
def test_fs_write_create_existing_makes_backup(tmp_path):
    ...
    f.write_text("old")
    out = agent_tools.fs_write(
        {"command": "create", "path": str(f), "file_text": "new"},
        cwd=str(tmp_path))
    assert "[BACKUP=" in out
...
def test_verify_change_fail_present_missing(tmp_path):
    ...
    f.write_text("hello")
    out = agent_tools.verify_change(
        {"present_in": [{"path": str(f), "pattern": "nope"}]},
        cwd=str(tmp_path))
...
def test_dispatcher_success():
    status, text, _ = agent_tools.run_tool(
        "execute_bash", {"command": "echo hi"}, cwd=".")
    assert status == "success" and "hi" in text


def test_dispatcher_error_handling():
    status, text, _ = agent_tools.run_tool(
        "fs_write", {"command": "str_replace", "path": "/nonexistent",
                     "old_str": "a", "new_str": "b"}, cwd=".")
    assert status == "error"
