...
def test_maybe_diff_basic(tmp_path):
    ...
    text, changed = agent_runtime._maybe_diff(
        "fs_write", {"path": str(cur)}, str(bak))
...
def test_maybe_diff_identical(tmp_path):
    bak = tmp_path / "a.bak"; cur = tmp_path / "a"
    bak.write_text("same\n"); cur.write_text("same\n")
...
def test_action_log_with_diff(store):
    aid = store.log_action(
        "sd1", "fs_write", {"path": "/x"}, ok=True,
        file="/x", backup="/x.bak", diff="--- a\n+++ b\n@@ -1 +1 @@\n-x\n+y\n",
        tool_use_id="tu-1")
