...
def test_log_and_list_actions(store):
    aid = store.log_action("s1", "fs_write", {"path": "/x"}, ok=True,
                           file="/x", backup="/x.bak.1")
    assert aid > 0
...
def test_action_failure_recorded(store):
    aid = store.log_action("s1", "execute_bash", {"command": "false"},
                           ok=False, error="nonzero")
    a = store.get_action(aid)
...
def test_meta_set_get(store):
    store.set_meta("s2", "plan",
                   {"items": [{"text": "step 1", "status": "pending"}]})
