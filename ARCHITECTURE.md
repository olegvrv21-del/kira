...
|---|---|---|
| 1 | ~~**LLM vendor lock-in on Amazon Q**~~ | **Done.** `llm/` abstraction layer shipped: `base.py` (Message/ToolCall/ToolSpec/StreamEvent/Usage + `LLMProvider` protocol), `q_provider.py`, `mock_provider.py`, `__init__.get_provider()` selects via `KIRA_LLM_PROVIDER`. `_llm_one_shot` and `_run_subagent_silent` use canonical messages; main `run_agent` routed through `QProvider.stream_raw_body()` (escape hatch keeps Q-dict history in DB). Phase 3c.2 (full canonical inside `run_agent`) deferred. Coverage 94.5%. |
